...
import {
  Container, Card, Form, Button, Spinner, Modal, Col, Row
...
const OrderCustomerCustomPaymentPage = () => {
  const { id } = useParams();
      ...
      try {
        ...
        if (orderData.order_status === 'menunggu_verifikasi') {
          Swal.fire('Tidak Bisa Akses', 'Pesanan kamu belum diverifikasi oleh admin.', 'info')
            .then(() => navigate(`/history/custompc/${id}`));
          return;
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await api.post(`/orders/customer/custom-pc/${id}/payment-confirmation`, formData, {
...
export default OrderCustomerCustomPaymentPage;
