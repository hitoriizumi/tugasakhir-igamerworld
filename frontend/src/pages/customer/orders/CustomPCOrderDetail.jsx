...
const CustomPCOrderDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchOrder = async () => {
      try {
        const res = await api.get(`/orders/customer/custom-pc/${id}`);
  ...
  const subtotal = components.reduce((sum, comp) => sum + (comp.product?.price || 0) * comp.quantity, 0);
  const totalBelanja = subtotal + buildFee + shippingCost;
  ...
  const handleConfirmPayment = () => {
    if (order.order_status === 'menunggu_verifikasi') {
      ...
      navigate(`/history/custompc/${id}/payment`);
    }
  };
  ...
  const handleViewDelivery = () => {
    ...
    if (['menunggu_verifikasi', 'menunggu_pembayaran', 'diproses'].includes(status)) {
      ...
      navigate(`/history/custompc/${id}/delivery`);
    }
  };
    ...
    <>
      ...
      <div style={{ backgroundColor: '#1C1C1C', paddingTop: '100px', paddingBottom: '60px', minHeight: '100vh' }}>
        <Container style={{ maxWidth: '1280px', color: 'white' }}>
          <h3 className="mb-4">Detail Perakitan PC</h3>
          {loading ? (
            ...
            <Row>
              <Col md={5}>
                <Card style={{ backgroundColor: '#2A2A2A', color: 'white' }} className="p-3 mb-4">
                  <h5 className="mb-3">Informasi Order</h5>
              ...
              <Col md={7}>
                <div className="border p-3" style={{ borderColor: '#f5c518', borderWidth: '1px', borderStyle: 'solid' }}>
                    ...
                    >
                      <Card.Body className="d-flex align-items-center">
                        ...
                        <div>
                          ...
                          <div>Harga: {formatCurrency(item.product?.price || 0)}</div>
                          <div>Jumlah: {item.quantity}</div>
                          <div>Subtotal: {formatCurrency((item.product?.price || 0) * item.quantity)}</div>
                        </div>
                      </Card.Body>
                    </Card>
                  ))}
                  ...
                  {buildFee > 0 && (
                    <div className="mt-3 d-flex justify-content-between text-warning">
                      <strong>Biaya Rakit oleh Toko</strong>
                      <strong>{formatCurrency(buildFee)}</strong>
                    </div>
                  )}
                  ...
                  <div className="d-flex justify-content-between align-items-center mt-3">
                    ...
                    <strong>
                      {formatCurrency(subtotal)} + {formatCurrency(buildFee)} + {formatCurrency(shippingCost)} = {formatCurrency(totalBelanja)}
                    </strong>
                  </div>
                </div>
              </Col>
...
export default CustomPCOrderDetail;
