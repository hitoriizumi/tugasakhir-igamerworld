...
import {
  Container, Card, Button, Spinner, Form, Row, Col
...
const OrderCustomerDeliveryPage = () => {
  const { id } = useParams();
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await api.get(`/customer/orders/product/${id}/delivery`);
  ...
  const handleConfirmReceived = async () => {
    ...
    try {
      await api.put(`/customer/orders/product/${id}/delivery/confirm`, {
        notes: receivedNote
      });
    ...
    <>
      ...
      <div style={{ backgroundColor: '#1C1C1C', paddingTop: '100px', paddingBottom: '60px', minHeight: '100vh' }}>
        <Container style={{ maxWidth: '1280px', color: 'white' }}>
          <h3 className="mb-4">Informasi Pengiriman</h3>
          ...
          {delivery?.order?.order_items?.length > 0 && (
            <>
              <h5 className="mt-5 mb-3">Produk yang Dibeli</h5>
              ...
              {/* 🚨 Alert Pre-order Warning */}
              {delivery.order.order_items.some(item => item.product?.status_stock === 'pre_order') && (
                <div className="alert alert-warning bg-warning bg-opacity-25 border-0 text-white">
                  Tidak ada split pengiriman. Semua produk akan dikirim jika seluruh item telah tersedia.
                </div>
              )}
              ...
              <div className="border p-3 mb-5" style={{ borderColor: '#f5c518', borderWidth: '1px', borderStyle: 'solid' }}>
                ...
                <div className="d-flex justify-content-between align-items-center mt-3">
                  ...
                  <strong>
                    {formatCurrency(
                      delivery.order.order_items.reduce((sum, i) => sum + Number(i.subtotal || 0), 0)
                    )} + {formatCurrency(Number(delivery.shipping_cost || 0))} (ongkir)
                    <span className="ms-2">= {formatCurrency(
                      delivery.order.order_items.reduce((sum, i) => sum + Number(i.subtotal || 0), 0) + Number(delivery.shipping_cost || 0)
                    )}</span>
                  </strong>
                </div>
              </div>
...
export default OrderCustomerDeliveryPage;
