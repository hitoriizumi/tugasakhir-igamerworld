...
const OrderCustomPCPaymentPage = () => {
  const { id } = useParams();
  ...
  const handleVerify = async (isVerified, note = '') => {
    ...
    try {
      ...
      MySwal.fire('Berhasil', `Pembayaran berhasil di${isVerified ? 'setujui' : 'tolak'}.`, 'success')
        .then(() => navigate(`/admin/orders/custom-pc/${id}`));
    } catch {
      ...
      <div style={{ paddingLeft: 80, paddingTop: 90, minHeight: '100vh' }}>
        <Container>
          {loading ? (
            ...
            <>
              <h4 className="fw-bold mb-4">Verifikasi Pembayaran Rakitan PC</h4>
...
export default OrderCustomPCPaymentPage;
