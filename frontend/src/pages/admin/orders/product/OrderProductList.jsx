...
const OrderProductList = () => {
  ...
  const handleApproveAll = async () => {
    const result = await MySwal.fire({
      title: 'Konfirmasi',
      text: 'Apakah Anda yakin ingin menyetujui semua pesanan biasa yang menunggu verifikasi?',
      icon: 'warning',
    ...
    // Cek apakah ada pesanan biasa dengan status menunggu verifikasi
    const hasPendingProductOrders = orders.some(
      (order) => order.order_status === 'menunggu_verifikasi' && order.order_type === 'product'
    );

    if (!hasPendingProductOrders) {
      await MySwal.fire({
        title: 'Tidak Ada Pesanan Biasa',
        text: 'Tidak ada pesanan biasa yang menunggu verifikasi.',
        icon: 'info',
    ...
    // Lanjut request ke API jika ada pesanan yang valid
    try {
      const res = await api.post('/orders/admin/product/approve-all');
      await MySwal.fire('Berhasil', res.data.message || 'Semua pesanan berhasil disetujui.', 'success');
      fetchOrders(); // refresh daftar pesanan
    } catch (error) {
      console.error(error);
      MySwal.fire('Gagal', error.response?.data?.message || 'Terjadi kesalahan saat menyetujui pesanan.', 'error');
    }
  };
      ...
      <div style={{ paddingLeft: '80px', paddingTop: '90px', minHeight: '100vh' }}>
        <Container>
          <div className="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-3">
            <h4 className="fw-bold">Daftar Semua Pesanan</h4>
            <div className="d-flex flex-wrap gap-2 mb-3">
              <Button as={Link} to="/admin/methods" variant="primary">
                Kelola Metode Pengiriman & Pembayaran
              </Button>
            </div>
          </div>
                        ...
                        <Link
                          to={
                            order.order_type === 'product'
                              ? `/admin/orders/product/${order.id}`
                              : `/admin/orders/custom-pc/${order.id}`
                          }
