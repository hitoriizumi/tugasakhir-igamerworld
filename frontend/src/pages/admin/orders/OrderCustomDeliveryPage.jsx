...
const OrderCustomDeliveryPage = () => {
  ...
  const fetchDelivery = useCallback(async () => {
    try {
      const res = await api.get(`/orders/custom-pc/${id}/delivery`);
  ...
  const handleSubmit = async () => {
    ...
    try {
      await api.post(`/orders/custom-pc/${id}/delivery`, payload, {
  ...
  const handleMarkAsFinished = async () => {
    ...
    try {
      await api.put(`/orders/custom-pc/${id}/delivery/finish`);
      ...
      <div style={{ paddingLeft: 80, paddingTop: 90, minHeight: '100vh' }}>
        <Container>
          {loading ? (
            ...
            <>
              <h4 className="fw-bold mb-4">Manajemen Pengiriman Rakitan PC</h4>
              ...
              <Card className="mb-4">
                <Card.Body>
                  <h5 className="mb-3 fw-bold">Komponen dalam Rakitan</h5>
                  <Table striped bordered responsive hover>
                    ...
                    <tbody>
                      {delivery.order.custom_p_c_order?.custom_pc_components?.map((item, index) => (
                        <tr key={item.id}>
                            <td>{index + 1}</td>
...
export default OrderCustomDeliveryPage;
